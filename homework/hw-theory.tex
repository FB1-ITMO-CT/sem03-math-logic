...
\item $R^\approx$ --- отношение нестрогого порядка на $A/_\approx$ в следующем смысле: $[a]_\approx R^\approx [b]_\approx$ выполнено, если $aRb$
...
%\item Напомним фрагмент доказательства корректности. 
%
%... Если $x \notin FV(\psi)$, тогда
...
%
%Восполните все пробелы в данном рассуждении.
...
\item Докажите, что теорема Гливенко в такой формулировке неверна в интуиционистском исчислении предикатов
(её можно переформулировать --- но это не входит в данное задание).
...
\item Пусть заданы какие-то дизъюнктные семейства термов без свободных переменных $T_1$ и $T_2$ (то есть
$T_1 \cap T_2 = \varnothing$), а также одноместный предикатный символ $P$.
Покажите, что семейство $\Gamma = \{ P(\theta)\ |\ \theta \in T_1 \} \bigcup \{ \neg P(\theta\ |\ \theta \in T_2 \}$ 
непротиворечиво.

%\item Пусть $M$ ---  полное непротиворечивое множество формул и $\mathcal{M}$ --- построенная в соответствии с теоремой о 
%полноте исчисления предикатов оценка для $M$. Мы ожидаем, что $\mathcal{M}$ будет моделью для $M$, для чего было 
%необходимо доказать
...
\item Попробуем наметить доказательство теоремы о переносе кванторов, рассмотрев некоторые вспомогательные
леммы. Несложно заметить, что используя данные и аналогичные утверждения, возможно доказать всю теорему:
\begin{enumerate}
\item Какая формула с поверхностными кванторами будет соответствовать формуле $(\forall x.P(x)) \vee \exists y.P(y) \with Q(y)$?
Докажите эквивалентность.
\item Эквивалентность предполагает наличие двух импликаций: для внесения кванторов внутрь --- или для вынесения их наружу.
Для начала вынесем квантор наружу --- например, для импликации: $(\forall x.\alpha)\rightarrow(\forall y.\beta)$. Как правильно вынести левый квантор, 
...
\item И теперь внесём квантор внутрь (например, для дизъюнкции):
$(\forall x.\alpha\vee\beta) \rightarrow (\forall x.\alpha)\vee(\forall x.\beta)$. 
Какие условия надо наложить на формулы $\alpha$ и $\beta$ (при наложении условия предложите обосновывающий его контрпример)?
\item Научимся преобразовывать выражение по частям: например, если $\alpha\rightarrow\beta$, 
то $(\beta\rightarrow\gamma) \rightarrow (\alpha\rightarrow\gamma)$ и $(\exists x.\alpha) \rightarrow (\exists x.\beta)$ 
(какие условия надо наложить на формулы $\alpha$ и $\beta$?).
\end{enumerate}
\end{enumerate}
...
\item вычитающую 1 из числа в двоичной системе (например, из $1011$ программа должна сделать $1010$);
...
\item Предложите метод, каким образом возможно закодировать машину Тьюринга с помощью двоичной строки.
Символы алфавитов занумеруйте (чтобы не иметь сложностей с разным начертанием букв).

\item На вашем любимом языке программирования напишите программу, печатающую свой текст.
Нельзя использовать рефлексию, работу с файлами и другие конструкций языка, дающие доступ к 
исходному коду. Данная программа (её аналог) используется в доказательстве неразрешимости
задачи останова, укажите это место.
...
\item Покажите, что в аксиоматике Пеано нет делителей нуля (нет положительных $p$ и $q$, что $pq = 0$),
и перенесите это доказательство в формальную арифметику: $\vdash p \cdot q = 0 \rightarrow p = 0 \vee q = 0$.

\item Покажите, что если $a \le b$ (в смысле определения выше), то $\vdash \exists t . \overline{a} + t = \overline{b}$.

\end{enumerate}

\section*{Задание №8. Арифметизация логики.}
\begin{enumerate}
\item Покажите, что какой-нибудь сильный модус (кроме Barbara) в арифметизации Лейбница всегда
корректен, а также что какой-нибудь слабый модус корректен, если термины непусты 
(и предъявите контрпример при пустых терминах).
\item Напомним, что $k$-местное отношение $R$ выразимо в формальной арифметике,
...
\item отношение $Z = \{ \langle x, 0 \rangle\ |\ x \in \mathbb{N}_0\}$.
\end{enumerate}
...
\section*{Задание №9. Теоремы Гёделя о неполноте арифметики. Теория множеств.}
...
теоремы о невыразимости доказуемости к множеству $\text{Д}_\mathcal{S}$ ведёт к противоречию.
...
\item Является ли порядок на алгебре Линденбаума полным? Если нет, то есть ли какие-нибудь
вполне упорядоченные бесконечные подмножества алгебры Линденбаума?
...
\begin{enumerate}
\item $\omega\cdot\overline{0} = \overline{0}\cdot\omega$
...
\end{enumerate}
\item При каких $a$ и $b$ выполнено $a + b = b$?
...
\item На лекции было приведено два различных определения для сложения и умножения ординалов (через порядковые типы и
индуктивное определение). Покажите, что эти определения эквивалентны.
\begin{enumerate}
...
%\item Канторовой нормальной формой для ординала назовём следующую запись:
%$\omega^{\beta_1} \cdot c_1 + \omega^{\beta_2} \cdot c_2 + \dots + \omega^{\beta_k} \cdot {c_k}$,
%где $c_i$ конечны, и $\beta_1 > \beta_2 > ... > \beta_k$ --- ординальные числа.
%Покажите, что возможно получить КНФ для ординала, построенного из конечных ординалов, $\omega$ с помощью операций
%сложения, умножения и возведения в степень. А именно, покажите следующее:
%\begin{enumerate}
%\item 
%\end{enumerate}

\item Покажите, что существует такой минимальный $\varepsilon_0$, что $\varepsilon_0 = \omega^\varepsilon_0$,
укажите его явный вид.
\end{enumerate}

\section*{Задание №11. Мощность множеств, аксиома выбора.}
...
$(\prec_M) = \bigcup\{\ (\prec)\ |\ \langle S, (\prec) \rangle \in X\}$ из предлагаемой теоремы 
...
\section*{Задание №12. Трансфинитная индукция. Теорема о непротиворечивости формальной арифметики}
\begin{enumerate}
...
\item Покажите, что $(\prec)$ на $\omega\times\omega$ из доказательства $|\omega\times\omega|=\omega$ --- отношение полного
порядка и имеет порядковый тип $\omega$.
\item Покажите, что $(\prec)$ в общем случае (из доказательства $|\alpha\times\alpha| = \alpha$) задаёт линейный порядок.
\item Покажите $\vdash_\infty\forall a.\forall b.\forall c.(a + b)\cdot c = a \cdot c + b \cdot c$.
\item Пусть $\alpha$ --- аксиома индукции в формальной арифметике. Покажите $\vdash_\infty |\alpha|_\infty$.
\item Пусть $\vdash\alpha$ в формальной арифметике. Поясните, почему можно найти такое доказательство $\vdash_\infty|\alpha|$, что
максимальная степень сечения в нём будет конечна? В данном задании требуется построить схему преобразования доказательства
из формальной арифметики в $S_\infty$, но конкретных доказательств преобразований конкретных аксиом и правил вывода приводить 
не обязательно --- достаточно оценить максимальную степень сечения для каждого.
\item Пусть $a \le b := \exists u.a + u = b$. Покажите, что $\vdash_\infty \forall a.\forall b.\forall c.a\le b \with b \le c \rightarrow a \le c$
(разумеется, частью решения является перевод формулы из языка формальной арифметики на язык $S_\infty$).
\item Существуют ли утверждения, доказательство которого не может иметь порядок, меньший $\omega$?
...
\section*{Задание №12а. Дополнительные задания по теории множеств}

\begin{enumerate}

\subsection*{Задачи на порядки}

\item Задано некоторое бинарное отношение $R\subseteq X^2$. Назовем \textit{циклом} такую последовательность $a_i\in X$, что $a_0Ra_1, a_1Ra_2,\dots,a_{n-1}Ra_n,a_nRa_0$. Докажите, что бинарное отношение без циклов может быть расширено до линейного порядка. (Примечание: для случая конечных множеств такое продолжение называется топологической сортировкой)

\item Докажите, что любой частичный порядок может быть расширено до линейного (формально: пусть $(X,\leqslant)$ -- частично упорядоченное множество, тогда существует $(\leqslant')\subseteq X^2$ -- линейный порядок такой, что $(\leqslant)\subseteq(\leqslant')$).

\item Пусть $\mathcal F$ -- множество финитных последовательностей натуральных чисел. Введем отношение $(\preceq)\subseteq \mathcal F^2$ -- отношение доминирования ($a\preceq b\Leftrightarrow \forall i\in\mathbb N.\,a_i\leqslant b_i$). Докажите, что $(F,(\preceq))\simeq (\mathbb Z,\{(a,b): \exists c:ac=b\})$.

\subsection*{Задачи на ординалы}

\item Арифметику ординалов можно задавать двумя способами: через рекурсивное определение и через порядковые типы (например, $a+b$ -- порядковый тип отмеченного объединения $a\uplus b$, $a\cdot b$ -- порядковый тип $a\times b$). На практике мы показали, что эти два определения эквивалентны (№10.6). Докажите следующие утверждения об ординалах двумя способами, принимая разные определения, не прибегая к их эквивалентности.
\begin{enumerate}
    \item Докажите, что $F(\alpha,\beta)=\{f:\alpha\to \beta\}$ -- множество всех функций из $\alpha$ в $\beta$ -- может быть вполне упорядочено так, чтобы порядковый тип эквивалентен $\beta^\alpha$ (далее принимайте эту формулировку в качестве определения степени через порядковые типы).
    ...
    Вычитание: пусть $\alpha\geqslant \beta$. Покажите, что существует единственный ординал $\gamma$ такой, что $\alpha=\beta+\gamma$. Выполнено ли аналогично утверждение для $\alpha=\gamma+\beta$?

    \item $(\alpha\cdot\beta)\cdot\gamma=\alpha\cdot(\beta\cdot\gamma)$;
    
    $1\cdot\alpha=\alpha\cdot1=\alpha$;
    ...
    \item Монотонность сложения и умножения:
    
    $\beta_1<\beta_2,\text{ тогда } \alpha+\beta_1<\alpha+\beta_2$;
    
    $\alpha_1<\alpha_2,\text{ тогда } \alpha_1+\beta\leqslant \alpha_2+\beta$;
    
    $\beta_1<\beta_2, \alpha\neq\varnothing,\text{ тогда } \alpha\cdot\beta_1<\alpha\cdot\beta_2$;

    $\alpha_1<\alpha_2,\text{ тогда } \alpha_1\cdot\beta\leqslant \alpha_2\cdot\beta$;
    
    Приведите примеры, когда во втором и четвертом случаях достигается равенство.
\end{enumerate}

\item Докажите следующие утверждения об ординалах.
\begin{enumerate}
    \item Докажите, что все конечные ординалы имеют вид $\varnothing^{\prime\prime\cdots\prime}$ (конечное число инкрементов).

    \item Для любого ординала $\gamma<\alpha\cdot\beta$ существуют единственные $\alpha'<\alpha$ и $\beta'<\beta$ -- ординалы -- такие, что $\gamma=\alpha\cdot\beta'+\alpha'$.
    
    Деление с остатком: $\alpha>0,\beta$ -- ординалы, тогда существуют единственные ординалы $\rho<\alpha$ и $\tau$ такие, что $\beta=\alpha\cdot\tau+\rho$.
    
    Системы счисления для ординалов: $\alpha>0$, тогда любой ординал $\gamma<\alpha^{k+1}$ ($k$ конечно) существуют единственные <<цифры>> $\beta_i<\alpha$ -- ординалы такие, что $\alpha=\alpha^k\cdot\beta_k+\alpha^{k-1}\cdot\beta_{k-1}+\dots+\alpha_0$.
    
    \item Пусть $\alpha\geqslant 2$. Докажите, что $\alpha^\beta\geqslant \alpha\cdot\beta$.
    
    Пусть $\alpha>1$. Найдите наименьшее решение уравнения $\alpha\cdot\beta=\beta$ в ординалах.
    
    \item Пусть $\alpha,\beta$ -- ординады. Докажите, что ординал $\gamma<\alpha^\beta$ тогда и только тогда, когда существуют $\beta_k<\beta_{k-1}<\dots<\beta_1<\beta$ и $\alpha_1,\dots,\alpha_k<\alpha$ такие, что $\gamma=\alpha^{\beta_1}\cdot\alpha_1+\cdots+\alpha^{\beta_k}\cdot\alpha_k$. Докажите, что такое представление $\gamma$ в виде суммы (называемое Канторовой нормальной формой) единственно.
\end{enumerate}

\subsection*{Задачи на фундированные множества}
\item Частично упорядоченное множество $X$, удовлетворяющее одному из пунктов (*)-(****), называется фундированным:

(*) В любом непустом подмножестве $Y\subseteq X$ найдется минимальный элемент.

(**) Не существует бесконечной строго убывающей последовательности $x_0>x_1>\cdots$ элементов $x_i\in X$.

(***) Любая невозрастающая последовательность стабилизируется: $$\forall x_0,x_1\dots\in X. (x_0\geqslant x_1\geqslant x_2\geqslant\dots)\to \exists i\in\mathbb N.\,\forall j\geqslant i.\,x_i=x_j$$

(****) (Принцип индукции) Для любого предиката $P$ выполнено $$\big(\forall x\in X. (\forall y. y<x\to P(y))\to P(x)\big)\to \forall x\in X.P(x)$$

\begin{enumerate}
    \item Докажите, что (*) $\Leftrightarrow$ (**).
    \item Докажите, что (**) $\Leftrightarrow$ (***).
    \item Докажите, что (*) $\Leftrightarrow$ (****).
\end{enumerate}

\item Пусть $A,B$ удовлетворяют определению (***) из предудыщего задания. Введем отношение $(\leqslant)$ на $A\times B$ 
так: $(a_1,b_1)\leqslant(a_2, b_2)$, если $(b_1<b_2)\vee(b_1=b_2\;\&\; a_1\leqslant a_2)$. 
Докажите, что это отношение является отношением порядка. Докажите, что $A\times B$ является фундированным, 
не используя доказательств из предыдущей задачи — и пользуясь только определением (***).

\item Пусть $X$ -- фундированное множество. Докажите, что существует единственная функция $\text{rg}$, принимающая элемент $x\in X$ и возвращающая ординал, такая, что для всех $x\in X$ выполнено $$\text{rg}(x)=\min\left\{\alpha\,|\,\,\forall y.\,y<x\to \text{rg}(y)<\alpha\right\}$$

\item Докажите, что если в любом непустом подмножестве частично-упорядоченного множества $X$ есть наименьший элемент, тогда заданный порядок автоматически является линейным.

\item Приведите пример фундированного, но не вполне упорядоченного множества.

\section*{Задачи на трансфинитную индукцию}
\item Пусть $X$ вполне упорядочено, $f:X\to X$ возрастает (т.е. $\forall x,y\in X. x<y\to f(x)<f(y)$). Докажите, что тогда $\forall x\in X.f(x)\geqslant x$.

\item Пусть $X$ вполне упорядочено, $Y$ -- произвольное множество; задано правило $F$, принимающее $x\in X$ и функцию $g:[0,x)\to Y$, возвращающее $F(x,g)\in Y$ (в некотором смысле, $F$ -- рекурсивное правило, называемое трансфинитной рекурсией). Формально, $F\subset X\times \bigcup\{\mathcal F([0,x),Y)\,|\,x\in X\} \times Y$ таково, что $$\forall x\in X.\,\forall g\in\mathcal F([0,x),Y). \exists ! y\in Y.\,(x,g,y)\in F$$
Будем говорить, что функция $f:X\to Y$ порождена правилом $F$, если $$\forall x\in X.\, f(x)=F\left(x,f|_{[0,x)}\right)$$

\begin{enumerate}
    \item Приведите правила, которые порождают: факториал на $\omega$; числа Фибоначчи на $\omega$; функцию Аккермана на $\omega^2$.
    \item Докажите, что существует единственная функция $f:X\to Y$, порождаемая рекурсивным правилом $F$.
\end{enumerate}

\item Пусть $(X,\preceq)$ -- полная решетка, заданная на множестве $X$, (решетка называется полной, если для любого $\varnothing\neq Y\subseteq X$ существует точные верхняя и нижняя границы). Функция $F:X\to X$ -- монотонна (т.е. $\forall x,y\in X.\,x\preceq y\to F(x)\preceq F(y)$). Докажите, что существует наименьшая неподвижная точка $F$, т.е. элемент $x\in X$, что $(F(x)=x)\,\&\,\forall y\in X.\, F(y)=y\to x\preceq y$.

\item Для предыдущей задачи докажите, что множество неподвижных точек образует решетку.

\end{enumerate}


\section*{Задание №13. Лямбда-исчисление}
...
\section*{Задание №14. Модальная, линейная темпоральная логика. Проверка на моделях}

\begin{enumerate}
    \item 
    Рассмотрим аксиоматическую систему модальной логики K.
    То есть, все схемы аксиом классического исчисления высказываний, 
    \emph{схему аксиом дуальности} $ \Diamond\varphi \leftrightarrow \neg\Box\neg\varphi$ 
    (стрелка в две стороны, как обычно, обозначает существование импликации в обе стороны), 
    а также \emph{схему аксиом нормальности (схему аксиом Крипке)}: 
    $ \Box(\varphi \to \psi) \to (\Box\varphi \to \Box\psi) $. 
    К \emph{Modus Ponens} добавим ещё одно правило вывода (его называют \emph{правилом необходимости}):
    \[ \infer{\Box\varphi}{\varphi}. \]
    ...
    \begin{enumerate}
        \item
        $ \vdash \varphi \to \Box\varphi$;

        \item 

        $ \vdash \Box\varphi \to \Box(\varphi \to \psi) $;
    
        \item 
        $ \vdash \Box(\varphi \land \psi) \to \Box\varphi $;
        
        \item
        $ \vdash \Box(\varphi \land \psi) \to \Box\psi $;

        \item 
        $ \vdash (\Box\varphi \land \Box\psi) \to \Box(\varphi \land \psi) $;
        ...
        \item 
        $ \vdash \Diamond\varphi \to \Diamond(\varphi \lor \psi) $;

        \item 
        $ \vdash (\Diamond\varphi \to \Box\psi) \to \Box(\varphi \to \psi) $.
    \end{enumerate}

\item Если исключить из ЛТЛ тавтологии с $\mathcal{U}$ и $\bigcirc$, верно ли, что
все оставшиеся тавтологии (то есть, тавтологии с $\Diamond$ и $\Box$) доказываются в S5?
\item Какие возможны варианты для переменной $\text{n}$ после исполнения программы 
в примере кода с лекции?
\item Рассмотрите семафор, окружающий некоторую критическую секцию (скажем, обновление
некоторой разделяемой переменной). Пусть эта секция выполняется в двух потоках.
Напишите формулу в линейной темпоральной логике, которая бы гарантировала корректную
работу семафоров.
\item Рассмотрим светофор на перекрёсте двух дорог, без допсекций. Будем считать,
что фаза зелёного длится 5 условных тактов, и 1 такт длится фаза жёлтого сигнала
(цикл примем таким: красный --- зелёный --- жёлтый --- красный).
Напишите формулу в ЛТЛ для показаний светофора и формулу от сигналов, задающую
условие безопасного движения: зелёный сигнал на пересекающихся улицах разделён
как минимум одним тактом красного для обоих направлений. Покажите в ЛТЛ, что для 
вашего алгоритма данное условие выполнено.
Выдерживается ли такое условие безопасного движения в реальном мире? (приведите
примеры конкретных перекрёстков).
\item Постройте недетерменированный обобщённый автомат Бюхи для $\bigcirc\alpha$
\item Постройте формулу для переменных $a$ и $b$, таких, что $a$ истинно в чётных 
тактах, а $b$ --- в нечётных, и постройте НОАБ для неё.
\end{enumerate}
